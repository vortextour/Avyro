...
fields.phone.addEventListener('input', (e) => {
  const val = e.target.value.trim();
  if (!isValidPhone(val)) return setStatus('phone', 'error', 'Invalid phone format (01X...).');
...
form.addEventListener('submit', async (e) => {
  ...
  if (hasValidationError) {
    formErrorMsg.textContent = "Please correct the highlighted errors above.";
  ...
  try {
    ...
    if (ref && ref !== MASTER_CODE) {
        ...
        } catch (chainErr) { console.error("Error building upline chain", chainErr); }
    }
