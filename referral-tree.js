...
import { doc, collection, onSnapshot, query, where, getDoc } from "https://www.gstatic.com/firebasejs/10.8.0/firebase-firestore.js";
...
// ==========================================
// 1-READ COST MAGIC: SINGLE QUERY DELTA SYNC
// ==========================================
const fetchTreeData = async (rootData) => {
    ...
    rootNode = {
        ...
        status: rootData.status, level: 0, joinDate: rootJoinDate, totalReferrals: rootData.successfulReferrals || 0, children: [],
    ...
    // ফায়ারবেসের শক্তিশালী Single Query যা ৩ লেভেলের সবাইকে একবারে আনবে!
    // Firebase Local Cache থাকার কারণে এই onSnapshot শুধু নতুন ডাটার জন্য Read কাটবে (Delta Sync)।
    const treeQuery = query(collection(db, "users"), where("uplineChain", "array-contains", rootData.referralCode));

    onSnapshot(treeQuery, (snapshot) => {
        snapshot.forEach(doc => {
            const data = doc.data();
            
            // Magic: uplineChain এর পজিশন দেখেই বোঝা যাবে সে কত নম্বর লেভেলে আছে
            let level = 1;
            if (data.uplineChain) {
                const idx = data.uplineChain.indexOf(rootData.referralCode);
                if (idx !== -1) level = idx + 1; // 0 = L1, 1 = L2, 2 = L3
            }
            
            const node = formatNode(data, level);
            treeDataMap.set(node.uid, node);
        });

        // Rebuild the Tree Hierarchy logically
        rootNode.children = [];
        Array.from(treeDataMap.values()).forEach(node => {
        ...
        Array.from(treeDataMap.values()).forEach(node => {
            ...
            // Find parent using uplineCode
            const parent = Array.from(treeDataMap.values()).find(p => p.referralCode === node.uplineCode);
            if (parent) {
                parent.children.push(node);
            } else if (node.level === 1) {
                // Direct downline fallback
        ...
        console.warn("Tree Sync Issue: Ensure existing users have 'uplineChain' arrays.", error);
    });
};
...
const openNodeModal = (uid) => {
    ...
    els.mJoinDate.textContent = node.joinDate ? new Date(node.joinDate).toLocaleDateString() : 'N/A';
